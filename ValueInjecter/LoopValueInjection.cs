    ...
    public abstract class LoopValueInjection<TSourceProperty, TTargetProperty> : PrefixedValueInjection
    {
        protected override void Inject(object source, object target)
        {
            ...
            for (var i = 0; i < source.GetProps().Count(); i++)
            {
                ...
                var value = s.GetValue(source, null);
                if (AllowSetValue(value))
                    t.SetValue(target, SetValue((TSourceProperty)value), null);
            }
        }
    ...
    public class LoopValueInjection : CustomizableValueInjection
    {
        ...
        protected override void Inject(object source, object target)
        {
            ...
            for (var i = 0; i < sourceProps.Count(); i++)
            {
                ...
                var t = target.GetProps().FirstOrDefault(prop => prop.Name == SearchTargetName(TargetPropName(s.Name)));
                if (t == null) continue;
                ...
                var value = s.GetValue(source, null);
                if (AllowSetValue(value))
                    t.SetValue(target, SetValue(value), null);
            }
        }
    }
}